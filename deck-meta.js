const arenaSelect=document.querySelector("#arena-select"),trophyInput=document.querySelector("#trophy-input"),detectArenaBtn=document.querySelector("#detect-arena-btn"),playerTagInput=document.querySelector("#meta-player-tag"),detectPlayerBtn=document.querySelector("#detect-player-btn"),sortSelect=document.querySelector("#sort-select"),timeSelect=document.querySelector("#time-select"),limitSelect=document.querySelector("#deck-limit-select"),refreshBtn=document.querySelector("#meta-refresh-btn"),stateEl=document.querySelector("#meta-state"),gridEl=document.querySelector("#meta-grid"),contextEl=document.querySelector("#meta-context");
async function fetchJson(url,options={}){const c=new AbortController(),timer=setTimeout(()=>c.abort(),22000);try{const r=await fetch(url,{...options,cache:"no-store",headers:{Accept:"application/json",...(options.headers||{})},signal:c.signal});const t=await r.text();let d=null;try{d=t?JSON.parse(t):null}catch{d={detail:t}}if(!r.ok)throw new Error(d?.detail||d?.error||`Erreur HTTP ${r.status}`);return d}catch(e){if(e?.name==="AbortError")throw new Error("Le serveur met trop de temps à répondre.");if(/Failed to fetch/i.test(String(e?.message||e)))throw new Error("Impossible de joindre l'API. Vérifie le Worker Cloudflare.");throw e}finally{clearTimeout(timer)}}
const ARENAS=[{"id": 1, "min": 0, "max": 300, "name": "Goblin Stadium"}, {"id": 2, "min": 300, "max": 600, "name": "Bone Pit"}, {"id": 3, "min": 600, "max": 1000, "name": "Barbarian Bowl"}, {"id": 4, "min": 1000, "max": 1300, "name": "Spell Valley"}, {"id": 5, "min": 1300, "max": 1600, "name": "Builder's Workshop"}, {"id": 6, "min": 1600, "max": 2000, "name": "P.E.K.K.A.'s Playhouse"}, {"id": 7, "min": 2000, "max": 2300, "name": "Royal Arena"}, {"id": 8, "min": 2300, "max": 2600, "name": "Frozen Peak"}, {"id": 9, "min": 2600, "max": 3000, "name": "Jungle Arena"}, {"id": 10, "min": 3000, "max": 3400, "name": "Hog Mountain"}, {"id": 11, "min": 3400, "max": 3800, "name": "Electro Valley"}, {"id": 12, "min": 3800, "max": 4200, "name": "Spooky Town"}, {"id": 13, "min": 4200, "max": 4600, "name": "Rascal's Hideout"}, {"id": 14, "min": 4600, "max": 5000, "name": "Serenity Peak"}, {"id": 15, "min": 5000, "max": 5500, "name": "Miner's Mine"}, {"id": 16, "min": 5500, "max": 6000, "name": "Executioner's Kitchen"}, {"id": 17, "min": 6000, "max": 6500, "name": "Royal Crypt"}, {"id": 18, "min": 6500, "max": 7000, "name": "Silent Sanctuary"}, {"id": 19, "min": 7000, "max": 7500, "name": "Dragon Spa"}, {"id": 20, "min": 7500, "max": 8000, "name": "Boot Camp"}, {"id": 21, "min": 8000, "max": 8500, "name": "Clash Fest"}, {"id": 22, "min": 8500, "max": 9000, "name": "PANCAKES!"}, {"id": 23, "min": 9000, "max": 9500, "name": "Valkalla"}, {"id": 24, "min": 9500, "max": 10000, "name": "Legendary Arena"}, {"id": 25, "min": 10000, "max": 10500, "name": "Lumberlove Cabin"}, {"id": 26, "min": 10500, "max": 11000, "name": "Royal Road"}, {"id": 27, "min": 11000, "max": 11500, "name": "Musketeer Street"}, {"id": 28, "min": 11500, "max": 12000, "name": "Summit of Heroes"}, {"id": 29, "min": 12000, "max": 12500, "name": "Magic Academy"}, {"id": 30, "min": 12500, "max": 13000, "name": "Ultimate Clash Pit"}, {"id": 31, "min": 13000, "max": 13500, "name": "Little Prince's Tavern"}, {"id": 32, "min": 13500, "max": 14000, "name": "Spirit Square"}];
const META_SCOPES={ranked:{label:"Classé — toutes les ligues"},"uc-all":{label:"Champion suprême — tous les ratings"},"uc-0-2000":{label:"Champion suprême — 0 à 1 999"},"uc-2000-2500":{label:"Champion suprême — 2 000 à 2 499"},"uc-2500-3000":{label:"Champion suprême — 2 500 à 2 999"},"uc-3000-plus":{label:"Champion suprême — 3 000+"},"2v2":{label:"2v2 — combats récents"},seasonal1:{label:"Voie saisonnière — Arène I"},seasonal2:{label:"Voie saisonnière — Arène II"},"seasonal-top":{label:"Voie saisonnière — 15 000+"},all:{label:"Toute la Voie des trophées"}};
...
function initArenaSelect(){const arena=[`<option value="all">Toute la Voie des trophées</option>`,...[...ARENAS].reverse().map(a=>`<option value="${a.id}">Arène ${a.id} — ${escapeHtml(a.name)} (${formatNumber(a.min)}–${formatNumber(a.max)})</option>`)].join("");arenaSelect.innerHTML=`<optgroup label="2v2"><option value="2v2">2v2 — combats récents</option></optgroup><optgroup label="Classé"><option value="ranked">Classé — toutes les ligues</option><option value="uc-all">Champion suprême — tous les ratings</option><option value="uc-0-2000">UC — 0 à 1 999</option><option value="uc-2000-2500">UC — 2 000 à 2 499</option><option value="uc-2500-3000">UC — 2 500 à 2 999</option><option value="uc-3000-plus">UC — 3 000+</option></optgroup><optgroup label="Voie saisonnière"><option value="seasonal1">Arène I — 14 000–14 499</option><option value="seasonal2">Arène II — 14 500–14 999</option><option value="seasonal-top">15 000+</option></optgroup><optgroup label="Voie des trophées">${arena}</optgroup>`}
...
async function loadMeta(){gridEl.innerHTML="";stateEl.className="state-msg";stateEl.textContent="Chargement des decks…";refreshBtn.disabled=true;try{await ensureCardCatalog();const params=new URLSearchParams({arena:arenaSelect.value,sort:sortSelect.value,time:timeSelect.value,limit:limitSelect.value,v:"11"});const tag=normalizePlayerTag(playerTagInput?.value||localStorage.getItem("evanlux:lastPlayerTag")||"");if(tag)params.set("seedTag",tag);const data=await fetchJson(`${API_BASE}/meta-decks?${params}`),decks=Array.isArray(data?.decks)?data.decks:[];if(!decks.length)throw new Error(arenaSelect.value==="2v2"?"Pas assez de combats 2v2 récents dans l'échantillon. Entre ton tag joueur et réessaie.":"Aucun deck disponible pour ce filtre.");renderContext(data);gridEl.innerHTML=decks.map((d,i)=>renderMetaDeck(d,i,data)).join("");bindImageFallbacks();bindStatsToggles();stateEl.className="state-msg state-msg--success";stateEl.textContent=`${decks.length} decks chargés${data.sampledBattles?` · ${formatNumber(data.sampledBattles)} combats analysés`:""}.`}catch(e){contextEl.innerHTML="";stateEl.className="state-msg state-msg--error";stateEl.textContent=e.message||"Impossible de charger les decks."}finally{refreshBtn.disabled=false}}

function renderContext(data){const chips=[];chips.push(data.scopeLabel||data.mode||"Decks");if(data.arena)chips.push(`Arène ${data.arena.id} — ${data.arena.name}`);if(data.sampledPlayers!=null&&data.sampledPlayers>0)chips.push(`${formatNumber(data.sampledPlayers)} joueurs`);if(data.matchingBattles!=null&&data.matchingBattles>0)chips.push(`${formatNumber(data.matchingBattles)} combats retenus`);if(data.estimated)chips.push("Échantillon élargi");contextEl.innerHTML=chips.map(x=>`<span class="meta-chip">${escapeHtml(x)}</span>`).join("")}
  ...
  return `
    <article class="meta-deck-card">
      <div class="meta-deck-head">
        ...
        <div class="meta-deck-title">
          ...
          <div class="meta-deck-stats">
            ...
            ${usage != null ? `<span>${formatNumber(usage)} parties</span>` : ""}
      ...
      <div id="${statsId}" class="meta-stats-panel" hidden>
        ...
        <p class="meta-stats-source">${data.source === "arena-source" || data.source === "deckshop-arena"
...
arenaSelect.addEventListener("change",loadMeta);sortSelect.addEventListener("change",loadMeta);timeSelect.addEventListener("change",loadMeta);limitSelect.addEventListener("change",loadMeta);refreshBtn.addEventListener("click",loadMeta);detectArenaBtn.addEventListener("click",detectArenaFromTrophies);detectPlayerBtn.addEventListener("click",detectFromPlayer);playerTagInput.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();detectFromPlayer()}});trophyInput.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();detectArenaFromTrophies()}});initArenaSelect();try{const t=localStorage.getItem("evanlux:lastPlayerTag");if(t)playerTagInput.value=`#${t}`}catch{}loadMeta();
